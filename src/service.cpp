...
DBusConnection *init_bus() {
  ...
  //与session dbus 建立连接
  // param１：bus type = {DBUS_BUS_SESSION, DBUS_BUS_SYSTEM} 一个系统dbus,
  // 一个普通用户dbus param２：错误信息，包括错误名与错误信息．
...
>>>>>>> 58e06b85287ce4119f44bd780328422e96bd3925
  ...
  //									type=
  //									member=
  //） param3: err info 只设置一个type =
  // signal,表示所有信号都接受．也可以加上接口，发送者bus_name
...
void handle_message(DBusConnection *connection) {
  ...
  while (1) {
    ...
    } else if (dbus_message_is_method_call(msg, "hello.world",
                                           "add")) { //处理 add　远程调用．
      DBusMessage *rp;
